...
class MongoLib {
  ...
  async porcadapersonasaldo (collection, persona) {
    const db = await this.connect()
    const objQuery = db.collection(collection).find({persona: persona})
    return db.collection(collection).find({persona: persona}).toArray()
  }
